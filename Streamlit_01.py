    ...
    if df_sp_raw is not None:
        if menu_selection == "📄 Báo Giá Phụ Tùng":
            ...
            if st.session_state.sub_action == "create":
                ...
                # --- PHẦN 1: THÔNG TIN KHÁCH HÀNG & CONTACT ---
                ...
                with r1_c2:
                    ...
                    # Hiển thị Phone/Email bằng Markdown để có chiều cao tương đương ô st.info
                    dt = f_conts[f_conts['Customer contact'] == selected_c].iloc[0] if list_conts and selected_c != "N/A" else {}
                    phone = dt.get('Phone','-'); email = dt.get('Email','-')
                    st.markdown(f"<div style='background-color:#f0f2f6; padding:10px; border-radius:5px; height:52px; border-left:5px solid #0068c9'>📞 {phone} <br> ✉️ {email}</div>", unsafe_allow_html=True)

                # --- PHẦN 2: ÉP THẲNG HÀNG MACHINE VÀ NGƯỜI LẬP ---
                ...
                if add_btn and input_search:
                    ...
                    if found_items:
                        st.session_state.cart.extend(found_items)
                        st.toast(f"✅ Đã thêm {len(found_items)} mã!")
                    if missing_codes:
                        st.error(f"⚠️ Không tìm thấy các mã sau: {'; '.join(missing_codes)}")
                ...
                if st.session_state.cart:
                    ...
                    st.data_editor(df_cart, column_config={"No": st.column_config.NumberColumn("No", width="small"), "Unit Price": st.column_config.NumberColumn("Unit Price", format="%,d"), "VAT": st.column_config.TextColumn("VAT", width="small"), "Qty": st.column_config.NumberColumn("Qty", min_value=1)}, use_container_width=True, hide_index=True, key="cart_editor_v8")
