...
def main():
    ...
    if menu_selection == "📄 Báo Giá Phụ Tùng":
        ...
        # --- BẢNG GIỎ HÀNG CÓ CỘT NO ---
        if st.session_state.cart:
            st.markdown("### 📋 Danh sách Part đã chọn")
            df_cart = pd.DataFrame(st.session_state.cart)
            df_cart.insert(0, 'No', range(1, len(df_cart) + 1)) # Thêm cột STT
            ...
            with btn_col2:
                st.button("✅ Xác nhận đặt hàng", use_container_width=True)
