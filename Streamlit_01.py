...
# --- 3. HÀM PRINT PDF (TÍNH NĂNG MỚI - TÁCH BIỆT) ---
def print_to_sample_sheet(header, cart_data, shipment, vat_total):
    try:
        ...
        for idx, row in enumerate(cart_data):
            rows.append([
                idx + 1, row["Part Number"], row["Part Name"], row["Qty"], row["Unit"],
                "", "", row["Unit Price"], row["% Discount"], "", row["Amount"],
                "", "", "", "", "", "", "", "", "", "", row["VAT"]
            ])
        ...
        st.success("Đã xuất sang 'Offer Sample' thành công!")
    except Exception as e:
    ...
    if st.session_state.page_view == "New":
        ...
        if col_btn1.button("Add to Cart", type="primary", use_container_width=True):
            if search_input:
                ...
                for code in [c.strip() for c in search_input.split(';')]:
                    ...
                    if not match.empty:
                        item = match.iloc[0]
                        st.session_state.cart.append({"Part Number": str(item.iloc[1]).split('.')[0], "Part Name": str(item.iloc[4]), "Qty": 1, "Unit": str(item.iloc[7]), "VAT": 8, "Unit Price": int(float(item.iloc[18])) if not pd.isna(item.iloc[18]) else 0, "% Discount": 0})
                    else: not_found.append(code)
                st.session_state.search_error = f"Không tìm thấy: {', '.join(not_found)}" if not_found else ""
                st.rerun()
        ...
        if st.session_state.cart:
            ...
            with col_sum2:
                ...
                st.table(pd.DataFrame({"Description": ["Total Amount", "Shipment Cost", "Sub-Total", "VAT Total", "Grand Total"], "Value": [total_amount, shipment, total_amount+shipment, total_vat, total_amount+shipment+total_vat]}).style.format({"Value": "{:,.0f}"}))
