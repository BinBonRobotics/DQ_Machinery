...
def load_all_data():
    try:
        ...
        # Đồng nhất kiểu dữ liệu String cho Offer_No để so sánh chính xác
...
def main():
    ...
    if st.session_state.menu_action == "Tạo báo giá":
        ...
        # --- HÀNG 2: MACHINE NUMBER & NGƯỜI LẬP ---
                ...
                for code in codes:
                    ...
                    if not match.empty:
                        ...
                        st.session_state.cart.append({
                            "Offer_No": current_off_id, # Tự động điền Offer_No
        ...
        # --- HIỂN THỊ GIỎ HÀNG & TỔNG CỘNG ---
        if st.session_state.cart:
            df_cart = pd.DataFrame(st.session_state.cart)
            # Sắp xếp Offer_No lên đầu
            ...
            if st.button("💾 Lưu Báo Giá", use_container_width=True, type="primary"):
                ...
                new_h_row = pd.DataFrame([{
                    ...
                    "Total_Amount": total_amt, "Shipment_Cost": ship_val, 
                    "VAT_Amount": vat_amt, "Grand_Total": grand_total
                }])
                df_h_final = pd.concat([df_h_stored[df_h_stored['Offer_No'] != current_off], new_h_row], ignore_index=True)
                
                # 2. Cập nhật Offer_Details
                ...
                cols_save = ['Offer_No','Part_Number','Part_Name','Qty','Unit','Unit_Price','VAT_Rate','Discount_Percent','Amount']
                df_d_final = pd.concat([df_d_stored[df_d_stored['Offer_No'] != current_off], details[cols_save]], ignore_index=True)

                # 3. Đẩy dữ liệu
                update_sheet("Offer_Header", df_h_final)
                update_sheet("Offer_Details", df_d_final)
                
                st.success(f"✅ Đã lưu thành công Báo giá: {current_off}")
                st.cache_data.clear(); st.rerun()
    ...
    # --- ORDER MANAGEMENT ---
        ...
        with tab1:
            ...
            sel_off = st.selectbox("Chọn Offer No để sửa:", [""] + list(df_h_stored['Offer_No'].unique()))
            if sel_off and st.button("📝 Sửa báo giá này", use_container_width=True):
                h_row = df_h_stored[df_h_stored['Offer_No'] == sel_off].iloc[0]
                st.session_state.edit_mode_header = h_row.to_dict()
                st.session_state.ship_cost = h_row['Shipment_Cost']
                
                d_rows = df_d_stored[df_d_stored['Offer_No'] == sel_off]
